# v8.3.1 Regime Quality Filter 연구 메모

## v8.3.0에서 확인된 핵심

동일 90일 ALL / Pattern ON / Regime ON / Score 75에서 전체 PF는 약 0.92였습니다. Regime별 분해 결과는 다음과 같았습니다.

- STRONG_BULL: PF 1.11 / 기대값 +0.061%
- BULL: PF 0.97 / 기대값 -0.014%
- RANGE: PF 0.54 / 기대값 -0.335%

Score별로는 75~79 구간이 PF 1.36이었고, 80 이상 구간은 오히려 악화되었습니다. 따라서 v8.3.1의 가설은 "점수가 높을수록 좋다"가 아니라 **좋은 대세 + 좋은 알트 상대강도 + 추격하지 않는 진입**입니다.

## v8.3.1 검증 가설

1. RANGE 신규 BUY를 차단하면 반복 반등매수와 거래비용이 줄어 PF가 개선되는가.
2. 15분 BTC 대비 상대강도가 약한 알트를 제거하면 BTC만 강한 장에서의 손실이 줄어드는가.
3. 높은 Score가 급등 직후 추격진입을 의미하는 경우 Anti-Chase가 손익을 개선하는가.
4. 상승장에서 Soft BTC Risk를 즉시 긴급청산하지 않고 복수 약화를 기다리면 Runner 수익을 더 보존하는가.

검증은 반드시 동일 90일·동일 수수료·동일 슬리피지·Score 75에서 `v8.3.0 비교`와 `v8.3.1 Quality`를 나란히 실행합니다. 목표 PF 1.0은 통과선일 뿐 수익 보장이 아니며, 30/60/90일과 워크포워드에서 재현되어야 합니다.
